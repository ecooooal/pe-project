...
class MailController extends Controller
{
    ...
    {
        $reviewerFiles = DB::table('reviewers')
            ->select('id', 'name', 'topic', 'path', 'created_at')
        ...
        return view('reviewers.index', [
            'headers' => ['ID', 'Subject', 'File Name', 'Date Created', 'Actions'],
            'rows' => $rows,
        ]);
    }
    ...
    public function index(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'reviewerFile.*' => 'required|file|mimes:pdf|max:20480'
        ], [
            'reviewerFile.*.max' => 'Each PDF file must not exceed 20MB.',
